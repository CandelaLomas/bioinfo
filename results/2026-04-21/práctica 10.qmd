...
```{´´´{bash}
zcat ../../data/2026-04-21/At4.10.fa.gz | grep -c "^>"
´´´
```
...
```{´´´{bash}
zcat ../../data/2026-04-21/At4.10.fa.gz | awk 'BEGIN {min=1e9; max=0; sum=0; n=0} 
    /^>/ {if(seq!="") {len=length(seq); sum+=len; n++; if(len<min) min=len; if(len>max) max=len; seq=""}} 
    !/^>/ {seq=seq$0} 
    END {len=length(seq); sum+=len; n++; if(len<min) min=len; if(len>max) max=len; 
...
```{´´´{bash}
zcat ../../data/2026-04-21/At4.10.fa.gz | awk '!/^>/ {sum+=length($0)} END {print "Total de bases:", sum}'
´´´
```
...
if [ ! -e asm1/At4.bp.p_ctg.gfa ]; then
   hifiasm -o asm1/At4 -t 2 -f 0 ../../data/2026-04-21/At4.10.fa.gz 2> asm1/hifiasm.log
fi
´´´
```
...
```{´´´{bash}
export HF = ~/bin/hifiasm/hifiasm :$PATH
$HF
´´´
```
