...
library(phangorn)
mtdna <- read.phyDat('../../results/2026-03-31/mtDNA.fasta', format = 'fasta')
```
...
1.  ¿Puedes confirmar el número de secuencias? ¿Qué longitud tiene el alineamiento? ¿Qué función usarías para extraer los nombres de las secuencias?
...
Para saber qué longitud tiene el alineamiento se utiliza el siguiente comando:

```{r}
attr(mtdna, "nr")
```

Para extaer los nombres de las secuencias se utiliza el siguiente comando:
...
`add.scale.bar()` añade una barra de escala al árbol que representa la distancia genética y `axisPhylo()` añade un eje de distancias genéticas al gráfico.
...
Para ver si las distancias evolutivas se corresponden con las dataciones, se obtiene el R^2^ , que indica cómo de bien estas dataciones explican las distancias evolutivas. Como el valor es cercano a 0, las distancias no reflejan bien las edades.
...
Para ver qué secuencias se desvían más de la tendencia general, se calculan los residuos de la regresión y se ordenan por valor absoluto.
...
Una tasa de evolución diferente, porque algunas especies evolucionan más rápido que otras, hibridación entre especies y la actuación de la selección natural, entre muchas otras.
...
Se divide la distancia media entre humanos y chimpancés entre la distancia media entre humanos y neardentales.
...
Se cogen solo las filas y columnas de los humanos modernos, solo la parte de abajo de la matriz para no contar dos veces la misma comparación, y se calcula el promedio.
...
1.  ¿Cuánto se parecen las distancias estimadas a las distancias cofenéticas?
...
La correlación es bastante alta, por lo que las distancias cofenéticas se parecen mucho a las distancias originales.
...
1.  ¿Serías capaz de crear otro conjunto de datos, `mtdna28`, que sólo contenga secuencias del género *Homo*?
...
El modelo que tiene el BIC más bajo es el mejor, por lo que es el siguiente:
...
1.  El árbol no está enraizado. Como las secuencias no son todas de la misma época, utiliza `rtt()` para situar la raíz en el punto más compatible con la hipótesis de reloj molecular.
...
2.  ¿Podrías obtener el árbol más verosímil para el subconjunto de secuencias del género *Homo*?

```{r}
#| eval: false
#Primero se filtran las fechas para Homo.
fechas_homo <- fechas[mt_ml$tree$tip.label]

#Se enraiza el árbol.

mt_ml_rtt_homo <- rtt(mt_ml$tree, tip.dates = fechas_homo, objective = 'rms')
...
Está expresada en sustituciones por sitio por año, que es el número esperado de cambios por cada par de bases en un año.

3.  ¿Cuánto tiempo dice el árbol que divergimos de los chimpancés? (Puedes usar la función `cophenetic()`, puesto que en el árbol datado las longitudes de las ramas están en años).
